...
import (
	...
	"github.com/financial-manager/api/cmd/api/handlers/transaction/expense/create"
...
func TestHandler_Handle(t *testing.T) {
	...
	tx := buildDomainTransaction("tx-1", "acc-001", 100.0)
	...
	tests := []struct {
		...
		{
			name: "valid body returns 201 with created expense transaction",
			body: map[string]any{
				"account_id":  "acc-001",
				"category_id": "cat-001",
				"amount":      100.0,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, "/api/v1/transactions/expenses", bytes.NewReader(bodyBytes))
