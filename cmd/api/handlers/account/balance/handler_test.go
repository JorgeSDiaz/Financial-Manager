...
import (
	...
	"github.com/financial-manager/api/cmd/api/handlers/account/balance"
...
// balanceResponse mirrors the handler's unexported balanceResponse for test decoding.
...
func TestHandler_Handle(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "existing account returns 200 with balance",
			id:         "acc-1",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodGet, "/api/v1/accounts/"+tc.id+"/balance", nil)
